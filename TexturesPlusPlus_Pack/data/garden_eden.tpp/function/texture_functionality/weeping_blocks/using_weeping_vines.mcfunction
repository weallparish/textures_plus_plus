advancement revoke @s only garden_eden.tpp:right_clicked_weeping_vines
...
execute if score @s garden_eden.tpp.ray_success matches 1 as @e[name=garden_eden.tpp.raycast_location] at @s run function garden_eden.tpp:texture_functionality/weeping_blocks/make_weeping_block